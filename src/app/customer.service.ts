...
export class CustomerService{
  ...
  getLoyaltyPoints(purchaseAmount: number):number{
    ...
    if(purchaseAmount > 100){
      ...
      totalLoyaltyPointsEarned = loyaltyPointsOver100 + loyaltyPointsOver50 + this.customer.loyaltyPoints;
      return totalLoyaltyPointsEarned;
    } else if (purchaseAmount > 50 && purchaseAmount < 100){
      loyaltyPointsOver50 = (purchaseAmount - 50) * 1;
      totalLoyaltyPointsEarned = loyaltyPointsOver50 + this.customer.loyaltyPoints;
      return totalLoyaltyPointsEarned;
    } else{
      totalLoyaltyPointsEarned = this.customer.loyaltyPoints;
      return totalLoyaltyPointsEarned;
    }

  }

  createPurchases(date: string, amount: number, description: string, customer: Customer): any{
